...
namespace BHive
{
	...
	struct BHIVE_API TextureFactory : public IResourceFactory<Texture>
	{
		...
		static TexturePtr Create2D(const glm::uvec2 &size, const FTextureCreateInfo &info = {}, const ByteBuffer &data = {});

		static TexturePtr Create2DArray(const glm::uvec2 &size, const FTextureCreateInfo &info);

		static TexturePtr Create3D(const glm::uvec3 &size, const FTextureCreateInfo &info, const ByteBuffer &data = {});
