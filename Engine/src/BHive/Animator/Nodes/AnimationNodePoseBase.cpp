...
namespace BHive
{
    void AnimationNodePoseBase::Update(const AnimatorContext &context)
    {
        mLastUpdatePlayCounter = context.mPlayCounter;
    }

    float AnimationNodePoseBase::GetNextPhaseUnwrapped(const AnimatorContext &context) const
    {
    ...
    void AnimationNodePoseBase::ExecuteImpl(const AnimatorContext &context, std::any &out_result)
    {
    ...
    void AnimationNodePoseBase::ApplyNextPhase(const AnimatorContext &context)
    {
        const bool copy_phase = (mPhaseRules & EPhaseRules::COPY) != 0;
