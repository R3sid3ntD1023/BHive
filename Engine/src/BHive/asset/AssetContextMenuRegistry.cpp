...
namespace BHive
{
	AssetContextMenuRegistry::AssetContextMenuRegistry()
	{
		...
		{
			auto type_var = derived.get_metadata("SupportedTypes").get_value<std::unordered_set<AssetType>>();
	...
	void AssetContextMenuRegistry::RegisterAssetContextMenu(
		const std::unordered_set<AssetType> &types, const Ref<FAssetContextMenu> &menu)
	{
