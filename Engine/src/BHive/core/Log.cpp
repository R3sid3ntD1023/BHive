...
namespace BHive
{
	void Log::Init()
	{
		...
		sinks[1] = CreateRef<spdlog::sinks::basic_file_sink_mt>("BHive_Log.log", true);
		sinks[2] = CreateRef<spdlog::sinks::callback_sink_mt>([&](const auto &msg) { OnMessageLogged.invoke(msg); });
