...
#define MAKE_DESERIALIZE_FUNCTION_TRAIT(name, func, versioned)\
    ...
    template <typename TStream, typename T>
    const bool has_serialize_v = details::has_serialize<TStream,T>::value;
    ...
    template<typename TStream, typename T>
    struct is_serializable : std::integral_constant<bool, serialize_counter<TStream, T>::value == 1> {};

    template <typename TStream, typename T>
    const bool is_serializable_v = is_serializable<TStream, T>::value;

    template<typename TStream, typename T>
    struct is_deserializable : std::integral_constant<bool, deserialize_counter<TStream, T>::value == 1> {};

    template <typename TStream, typename T>
    const bool is_deserializable_v = is_deserializable<TStream, T>::value;
} // namespace BHive
